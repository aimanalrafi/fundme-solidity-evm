...
# make sure that keystore for wallet interacting with anvil exists. use cast wallet list in terminal to check
NETWORK_ARGS := --rpc-url http://localhost:8545 --account foundry-fundme --password $(KEYSTORE_PASSWORD) --broadcast

# make sure that keystore for wallet interacting with sepolia testnet exists
ifeq ($(findstring --network sepolia,$(ARGS)),--network sepolia)
	NETWORK_ARGS := --rpc-url $(SEPOLIA_RPC_URL) --account foundry-fundme --password $(KEYSTORE_PASSWORD) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif
